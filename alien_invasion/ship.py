import pygame
from pygame.sprite import Sprite

# Class to control the ship
class Ship(Sprite):
    def __init__(self, ai_game):
        super().__init__()
        # Initiate the ship and its starting position
        self.screen = ai_game.screen
        self.settings = ai_game.settings
        self.screen_rect = ai_game.screen.get_rect()

        # Load ship1.png and get rects (x,y)
        self.image = pygame.image.load(
            "Part II - Projects/Alien Invasion/alien_invasion/images/ship1.png")
        self.rect = self.image.get_rect()

        # Ship start in center/bottom
        self.rect.midbottom = self.screen_rect.midbottom

        # Decimal value to ship movement
        self.x = float(self.rect.x)
        self.y = float(self.rect.y)

        # Set two flags to continuous movement
        self.moving_right = False
        self.moving_left = False
        self.moving_up = False
        self.moving_down = False

    # Update position based on the flags
    # Updating the x value of the ship, not the rect
    def update(self):
        # X Position
        # Set the right border
        if self.moving_right and self.rect.right < self.screen_rect.right:
            self.x += self.settings.ship_speed
        # Set the left border
        if self.moving_left and self.rect.left > 0:
            self.x -= self.settings.ship_speed

        # Y Position
        if self.moving_up and self.rect.top > 0:
            self.y -= self.settings.ship_speed
        if self.moving_down and self.rect.bottom < self.screen_rect.bottom:
            self.y += self.settings.ship_speed

        # Update rect object from self.x
        self.rect.x = self.x
        self.rect.y = self.y

    def blitme(self):
        # Draw the ship in its position
        self.screen.blit(self.image, self.rect)

    def center_ship(self):
        self.rect.midbottom = self.screen_rect.midbottom
        self.x = float(self.rect.x)
